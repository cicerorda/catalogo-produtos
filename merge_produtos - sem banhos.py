...
# ===== Utils =====
def safe_str(v: object, default: str = "") -> str:
    return v if isinstance(v, str) else default

def class_prefix(cls: str, n: int = 7) -> str:
    cls = safe_str(cls)
    return cls[:n] if len(cls) >= n else cls

def extrair_blocos_aa_bb_ccc(cls: str):
    """ De CLASSIFICACAO -> (AA, BB, CCC) """
    cls = safe_str(cls)
...
def gerar_categoria_prefixo7_mais_descricao(cls: str, descricao: str) -> str:
    ...
    # mantém a descrição original (sem trocar espaços por _), como pedido
...
# ===== Mapa prefixo(7) -> Categoria (mais frequente no histórico) =====
...
# ===== Índice por Referencia para deduplicar/atualizar =====
antigos_by_ref = {}
for it in antigos:
...
atualizados = 0
categorias_criadas = set()  # categorias geradas por não existirem no antigo
...
for raw in novos:
    ...
    # 0) validações mínimas
    if not (cls and ref and desc):
        skip_sem_campos += 1
        continue

    # 1) filtro por DESCRICAO: deve conter "BRUTA" OU "BANHO"
    desc_upper = desc.upper()
    if ("BRUTA" not in desc_upper) and ("BANHO" not in desc_upper):
        skip_desc_sem_palavra_chave += 1
        continue

    # 2) filtro por prefixo de 4 dígitos da CLASSIFICACAO
    if cls[:4] not in ALLOWED_PREFIXES_4:
        skip_prefixo_nao_permitido += 1
        continue

    # 3) Categoria: tenta pelo prefixo de 7; se não houver, cria "AA_BB_CCC_<DESCRICAO>"
    ...
    if not categoria:
        ...
        # registra no mapa para próximos itens do mesmo prefixo
    ...
    # 4) Mescla: atualiza se já existir, senão adiciona
...
print(f"➕ Itens novos adicionados (após filtros): {len(novos_convertidos)}")
